...
export const componentCatalogEntries: Record<string, ComponentCatalogEntry> = {
  ...
  DateInput: {
    ...
    props: [
      ...
      p('range', 'boolean', 'false', 'Select a from/to date range with two months side by side'),
      p('showTime', 'boolean', 'false', 'Embed TimeInput; when range is true, time is also a range'),
      p(
        'maxRangeDays',
        'number',
        undefined,
        'Inclusive max days for range selection (omit / ≤0 = unlimited)',
      ),
      ...
      p('todayLabel', 'string', undefined, 'Label for the today button in single mode (defaults by locale)'),
      p('confirmLabel', 'string', undefined, 'Label for the confirm button in range mode (defaults by locale)'),
      p('id', 'string', undefined, 'Input element id'),
  ...
  SidebarMenuGroup: {
    ...
    events: [
      e('click', 'string', 'Fired with the group id when the trigger is clicked'),
      e('select', 'string', 'Fired with the group id when the trigger is clicked (alias of click)'),
    ],
  },
  ...
  AppLayout: {
    ...
    slots: [
      ...
      s('menu', undefined, 'SidebarMenuItem / SidebarMenuGroup — set id equal to settingsMenuId on a group or lone item to pin it to the footer when settingsMenu is true'),
    ...
    composition: {
      ...
      parts: [
        ...
        c('flyoutPlacement on settings group', 'Use :flyout-placement="\'up\'" so the pinned settings flyout opens upward', true),
